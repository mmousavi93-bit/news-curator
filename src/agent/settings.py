...
def _cross_section_errors(built: dict[str, Any]) -> list[str]:
    ...
    thresholds, but the per-leaf check above only verifies type and
    non-negativity -- it cannot see relationships BETWEEN sections. The
    checks moved to settings_guard.py 2026-09-09 (session 9s) when the
    batch-vs-tpm guard pushed this file over the ~200-line cap; this
    wrapper stays so the error texts keep one import site."""
    from agent.settings_guard import cross_section_errors

    return cross_section_errors(built)
